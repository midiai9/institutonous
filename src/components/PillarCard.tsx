...
export default function PillarCard({ pillar, index }: { pillar: Pillar; index: number }) {
  ...
  return (
    <article className="group relative flex h-full flex-col overflow-hidden rounded-3xl bg-white p-7 shadow-card ring-1 ring-navy/5 transition-transform duration-300 hover:-translate-y-1.5">
      ...
      <div className={`mb-5 flex h-14 w-14 items-center justify-center rounded-2xl transition-transform duration-300 group-hover:-rotate-6 group-hover:scale-110 ${ICON_BG[index % 4]}`}>
        <Icon size={28} strokeWidth={2} className="transition-transform duration-300 group-hover:scale-110" />
      </div>
